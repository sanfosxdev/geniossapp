...
import DeleteCustomerCategoryConfirmationModal from './DeleteCustomerCategoryConfirmationModal';

interface ManageCustomerCategoriesModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const ManageCustomerCategoriesModal: React.FC<ManageCustomerCategoriesModalProps> = ({ isOpen, onClose }) => {
  const [categories, setCategories] = useState<CustomerCategory[]>([]);
  const [editingCategory, setEditingCategory] = useState<CustomerCategory | null>(null);
  const [categoryToDelete, setCategoryToDelete] = useState<CustomerCategory | null>(null);
  ...
  const handleEditClick = (category: CustomerCategory) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing && editingCategory) {
      await updateCustomerCategory({ ...editingCategory, name, color });
    } else {
      await addCustomerCategory({ name, color });
    }
    fetchCategories();
    clearForm();
  };

  const handleDeleteClick = (category: CustomerCategory) => {
    setCategoryToDelete(category);
  };

  const handleConfirmDelete = async () => {
    if (categoryToDelete) {
      await reassignCustomersFromCategory(categoryToDelete.id);
      await deleteCustomerCategory(categoryToDelete.id);
      fetchCategories();
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 animate-fade-in">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl transform animate-slide-in-up max-h-[90vh] flex flex-col">
          <header className="flex justify-between items-center p-5 border-b dark:border-gray-700">
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Administrar Categorías de Clientes</h2>
      ...
      <DeleteCustomerCategoryConfirmationModal
...
export default ManageCustomerCategoriesModal;