...
const ROLE_PANELS: Record<UserRole, AdminPanel[]> = {
  [UserRole.DEV]: ['orders', 'reservations', 'tables', 'products', 'customers', 'users', 'schedule', 'bots', 'settings'],
  [UserRole.ADMIN]: ['orders', 'reservations', 'tables', 'products', 'customers', 'users', 'schedule', 'bots', 'settings'],
  [UserRole.MOZO]: ['orders', 'reservations', 'tables', 'customers'],
  [UserRole.COCINA]: ['orders'],
};
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onGoToSite, onSliceBotStatusChange, currentUserEmail, currentUserRole, onLogout }) => {
  const allowedPanels = ROLE_PANELS[currentUserRole] || ['orders'];
  const [activePanel, setActivePanel] = useState<AdminPanel>(allowedPanels[0]);
    ...
    if (!allowedPanels.includes(activePanel)) {
      return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 text-center">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Acceso restringido</h2>
          <p className="mt-2 text-gray-600 dark:text-gray-300">Tu rol no tiene permisos para este panel.</p>
        </div>
      );
    }
